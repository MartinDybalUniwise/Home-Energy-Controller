...
- [ ] AC-019-001: At 1920x1080, Today has no vertical or horizontal page
  overflow and every required section is fully visible without clipping or
  overlap.
- [ ] AC-019-002: The desktop sidebar is 190-210 px, preserves every route,
  provides 52-58 px navigation rows, exposes compact health, and remains
  keyboard- and touch-operable.
- [ ] AC-019-003: The header is 100-120 px and includes required concise
  greeting/weather information without duplicate PV messaging.
- [ ] AC-019-004: One 90-110 px recommendation panel shows the best window plus
  concise Now, Avoid, and PV-peak statuses; separate status cards do not render.
- [ ] AC-019-005: All six telemetry items remain visible, data-bound,
  semantically colored, and readable in a 90-105 px strip.
- [ ] AC-019-006: Daily Energy Rhythm is 260-290 px, uses expanded width, retains
  required timeline details, and shows production estimate/peak once in its
  header.
- [ ] AC-019-007: The lower workspace is 300-330 px and contains exactly the
  compact appliance panel and chart at approximately 35-40% / 60-65%; the
  standalone next-window donut does not render.
- [ ] AC-019-008: Appliance rows are 52-60 px and retain icon, name, time, and
  status. No non-functional action is presented as operative.
- [ ] AC-019-009: The chart retains PV, consumption, price, and all three horizon
  controls with legible rendering.
- [ ] AC-019-010: Czech/English render without unknown keys, missing-data states
  use existing fallbacks, and no fake business logic is added.
- [ ] AC-019-011: Non-Today routes and shared sidebar behavior pass smoke checks
...
- [ ] AC-019-013: Safety settings remain disabled, no physical write occurs, and
