...
- [ ] AC-022-001: A poll longer than its configured interval does not cause an
	immediate scheduler catch-up loop.
- [ ] AC-022-002: A slow or failing SDG reader does not block healthy readers or
	terminate the scheduler.
- [ ] AC-022-003: Scheduler stop behavior is bounded, deterministic, and tested.
- [ ] AC-022-004: Runtime diagnostics identify process/checkout context,
	reader initialization, poll duration, interval, and latest SDG state without
	exposing secrets.
- [ ] AC-022-005: Existing Step 21 `sdg_history` parsing, checkpoint,
...
- [ ] AC-022-006: Read-only validation against `T:\Home-Energy-Controller` and
	...
	change.
- [ ] AC-022-007: Controller, TNG, and GoodWe write gates remain disabled in
	local, preview, and automated validation.
- [ ] AC-022-008: Focused tests, Ruff, relevant pytest, safe preview/browser
