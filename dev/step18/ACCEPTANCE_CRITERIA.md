...
- [x] AC-018-001: 1920x1080 Full HD rozvržení bez zbytečného scrollování a s
  efektivním využitím celé plochy (zero waste space).
- [x] AC-018-002: Vysoká čitelnost hlavních hodnot a stavů z 1–2 metrů
  (dominantní čísla 64–96 px, základní text 18–22 px, kontrast >= 4.5:1).
- [x] AC-018-003: Dotykové ovládání s cílovou velikostí interaktivních prvků
  min. 64×64 px, mezerami 12–16 px a zamezením jalových prázdných ploch.
- [x] AC-018-004: Zachování všech současných funkcí, datových toků, API
  kontraktů a bezpečné chování při výpadku externích dat.
- [x] AC-018-005: Kompletní jazyková lokalizace CZ a EN pro všechny nové texty,
  popisky a dialogy bez natvrdo vepsaných řetězců.
- [x] AC-018-006: Správné zobrazení a chování stránky Today (`Dnes`) podle
...
- [x] AC-018-007: Playwright E2E testy PASS napříč definovanými viewporty
  (1920x1080, 1440x900, 1280x800, 1024x768, 390x844) s nulovými chybami
  v konzoli.
- [x] AC-018-008: Plná validace repozitáře a SDD kroků (`dev/sdd/tools/validate_step.py`
  a `dev/sdd/tools/full_validation.py`) PASS.
- [x] AC-018-009: Formální schválení Gate A před implementací a úspěšný
